...
SickS300F::SickS300F() : param_node_("~"), nodeHandle_("/") {

  double x, y, z;

  // reading transformation parameters from parameter server
  param_node_.param(std::string("frame"), frame_id_, std::string("base_laser_link_F"));
  param_node_.param<bool>(std::string("send_transform"), send_transform_, 1);

  param_node_.param(std::string("tf_x"), x, 0.73585);
  ...
  double fov;
  param_node_.param(std::string("field_of_view"), fov, 190.0);
  if ((fov > 270) || (fov < 0)) {
  ...
  param_node_.param(std::string("devicename"), device_name_, std::string("/dev/ttyS0"));
  baud_rate_ = (unsigned int) param_node_.param(std::string("baudrate"), 500000);

  scan_data_publisher_ = nodeHandle_.advertise<sensor_msgs::LaserScan>("laserscanF", 10);
}

SickS300F::~SickS300F() {
}

void SickS300F::update() {
...
void SickS300F::broadcast_transform() {
  if (send_transform_) {
    tf_broadcaster_.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(1, 0, 0, 0), transform_vector_),
