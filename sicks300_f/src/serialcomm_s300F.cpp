...
SerialCommS300F::SerialCommS300F() {
  m_rxCount = 0;
  m_ranges = NULL;
}
...
int SerialCommS300F::connect(const std::string &deviceName, unsigned int baudRate) {

  m_fd = ::open(deviceName.c_str(), O_RDWR | O_NOCTTY);
  if (m_fd < 0) {
    ROS_ERROR_STREAM("SerialCommS300F: unable to open serial port " << deviceName);
    return -1;
  }
...
void SerialCommS300F::setFlags() {
  ...
  if (tcsetattr(m_fd, TCSANOW, &term) < 0)
    ROS_ERROR("SerialCommS300F: failed to set tty device attributes");
  tcflush(m_fd, TCIOFLUSH);
}

int SerialCommS300F::setBaudRate(int baudRate) {

  struct termios term;

  if (tcgetattr(m_fd, &term) < 0) {
    ROS_ERROR("SerialCommS300F: unable to get device attributes");
    return 1;
  }

  if (cfsetispeed(&term, baudRate) < 0 || cfsetospeed(&term, baudRate) < 0) {
    ROS_ERROR_STREAM("SerialCommS300F: failed to set serial baud rate " << baudRate);
    return 1;
  }

  if (tcsetattr(m_fd, TCSAFLUSH, &term) < 0) {
    ROS_ERROR("SerialCommS300F: unable to set device attributes");
    return 1;
  }
...
int SerialCommS300F::baudRateToBaudCode(int baudRate) {
  switch (baudRate) {
    case 38400:
      return B38400;
    case 115200:
...
int SerialCommS300F::disconnect() {
  ::close(m_fd);
  return 0;
}
...
unsigned short SerialCommS300F::createCRC(unsigned char *data, ssize_t length) {
...
int SerialCommS300F::readData() {

  int available_bytes;
  ioctl(m_fd, FIONREAD, &available_bytes);
  if (available_bytes > 1200) {
    ROS_WARN("SerialCommS300F: more than one packet is buffered, measurements are potentially stale!");
  }

  // if we do not have at least a header in buffer, read it
  if (m_rxCount < header_size) {
    ROS_DEBUG("SerialCommS300F: Reading header bytes");
  ...
  while (m_rxCount >= header_size) {
    ...
    for (packet_start = 0; packet_start <= m_rxCount - header_size && !found_header; ++packet_start) {
      if (memcmp(&m_rxBuffer[packet_start], "\0\0\0\0\0\0", 6) == 0 &&
        ...
        if (packet_start > 0) {
          ROS_WARN("SerialCommS300F: Warning: skipping %u bytes, header found", packet_start);
    ...
    if (!found_header) {
      ROS_WARN("SerialCommS300F: Warning: skipping %u bytes, header not found", packet_start);
    ...
    if (protocol == PROTOCOL_1_02) {
      ...
      ROS_ERROR_STREAM("SerialCommS300F: protocol version " << std::hex << protocol << std::dec << " is unsupported");
    ...
    if (total_packet_size > RX_BUFFER_SIZE) {
      ROS_ERROR("SerialCommS300F: Requested size of data is larger than the buffer size");
      discard_byte();
      continue;
    }
    ...
    if (status == -2) {
      ROS_ERROR("SerialCommS300F: Could not read the rest of the data packet!");
      return -2;
    }
    // give up for now if we don't have all bytes, return soft failure
    if (m_rxCount < total_packet_size) {
      ROS_DEBUG("SerialCommS300F: Giving up for now, have only %lu bytes", m_rxCount);
      return -1;
    }
    ...
    ROS_DEBUG("SerialCommS300F: Size: %d m_rxCount: %lu accessing checksum at m_rxBuffer[%u]", size, m_rxCount, total_packet_size - 2);

    if (packet_checksum != calc_checksum) {
      ROS_ERROR_STREAM("SerialCommS300F: Checksums don't match "<< calc_checksum<< "(data packet size " << size << ")");
      discard_byte();
      continue;
    } else { // checksum valid
      ROS_DEBUG_STREAM("SerialCommS300F: Checksums match (data packet size " << size << ")");

      if (m_rxBuffer[block_type_id_location] != m_rxBuffer[block_type_id_location + 1]) {
        ROS_ERROR("SerialCommS300F: Bad type header bytes don't match");
        ...
        if (m_rxBuffer[block_type_id_location] == 0xAA) {
          ROS_ERROR("SerialCommS300F: We got an I/O data packet, we don't know what to do with it");
          ...
          if (beam_count < 0) {
            ROS_ERROR_STREAM("SerialCommS300F: bad beam count (" << beam_count << ")");
            discard_byte();
            continue;
          }
          ...
          ROS_ERROR("SerialCommS300F: We got a reflector data packet, we don't know what to do with it");
        } else {
          ROS_ERROR("SerialCommS300F: We got an unknown packet");
        }
      }
    }
...
int SerialCommS300F::read_byte(unsigned int count) {
  ssize_t len = read(m_fd, &m_rxBuffer[m_rxCount], count);

  if (len < 0) {
    ROS_ERROR_STREAM("SerialCommS300F: error reading from serial port: " << strerror(errno));
    return -2;
  } else if (len == 0) {
    ROS_WARN("SerialCommS300F: received zero bytes!");
...
void SerialCommS300F::discard_byte(unsigned int count) {
